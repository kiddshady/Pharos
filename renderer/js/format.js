...
/**
 * Un precio argentino: $14.189,89
 *
 * fmtMoney no sirve para esto y no es un descuido suyo: no agrupa miles (a
 * propósito, porque lo que pasa por ahí ya viene reducido a menos de mil) y
 * antepone el código de moneda. Un precio de góndola necesita lo contrario —
 * el signo pegado y los miles separados. Sin agrupar, "14189,89" obliga a
 * contar dígitos, que es justo lo que uno no quiere hacer comparando remedios.
 *
 * Con es-AR el separador de miles es el punto y el decimal la coma, que es
 * exactamente como los escribe el Manual Farmacéutico.
 */
export function fmtPesos(n, { signo = '$' } = {}) {
  if (n == null || !Number.isFinite(Number(n))) return '—';
  const f = new Intl.NumberFormat(locale.tag, {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
    useGrouping: true,
  });
  return `${signo}${f.format(Number(n))}`;
}
