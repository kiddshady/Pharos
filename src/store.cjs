...
const ROOT = process.env.PHAROS_DATA || path.join(__dirname, '..', 'data');
...
const DEFAULT_SETTINGS = {
  ...
  /** El descuento que se aplica sobre los precios de lista. Queda guardado
      porque casi siempre es el mismo y reescribirlo en cada consulta es
      trabajo repetido. `activo` lo apaga sin perder el número. */
  descuento: { activo: false, porcentaje: 0 },

  /** Cuánto vale una respuesta guardada antes de volver a preguntar. Los
      precios del Manual se actualizan por día; medio día es el equilibrio
